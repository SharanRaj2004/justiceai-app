...
export default function NotFoundPage() {
  ...
  const suggestions = [
    { icon: Home, label: 'BASE_TERMINAL', path: '/', color: 'text-red' },
    { icon: MessageSquare, label: 'SYNC_AI_CHAT', path: '/chat', color: 'text-blue' },
    { icon: BookOpen, label: 'RIGHTS_STRATUM', path: '/rights', color: 'text-white' },
    { icon: Search, label: 'SEARCH_REGISTRY', path: null, color: 'text-red-light' },
  ];
      ...
      <main className="max-w-4xl mx-auto px-6 pt-48 pb-24 flex flex-col items-center text-center">
        ...
        >
          <h1 className="text-8xl md:text-9xl font-display font-bold text-white uppercase tracking-tighter italic">
            VOID_<span className="text-red">404</span>
          </h1>
          <div className="flex items-center gap-4 justify-center px-6 py-2 bg-red/10 border-2 border-red/20 rounded italic">
            <Terminal className="w-5 h-5 text-red" />
            <p className="text-[12px] uppercase font-extrabold tracking-[0.4em] text-red">
          ...
          {suggestions.map((item, i) => {
            ...
            return (
              <button
                ...
                className="group p-8 rounded bg-void border-2 border-white/5 hover:border-red/40 transition-all duration-300 flex flex-col items-center gap-4 shadow-hard hover:shadow-hard-red/20 active:translate-y-[2px]"
              >
                <div className="w-12 h-12 rounded bg-void border-2 border-white/10 flex items-center justify-center group-hover:border-red/40 group-hover:scale-110 transition-all">
        ...
        <motion.button
          ...
          onClick={() => navigate(-1)}
          className="flex items-center gap-3 text-[10px] font-extrabold text-red hover:text-white transition-all uppercase tracking-[0.4em] italic decoration-2 underline-offset-8 hover:underline"
        >
